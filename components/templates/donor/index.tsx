...
function DonorBuilder() {
  ...
  // --- HANDLERS ---
  const handleDataChange = (field: keyof DonorData, val: string) => {
        ...
        {templateId === 1 && (
          <div className="flex flex-col h-full">
            ...
            <div className="space-y-6 flex-grow text-justify px-1">
              ...
              <div className="ml-8 space-y-1.5 font-sans text-[10.5pt] border-l-4 border-red-100 pl-6 italic break-inside-avoid">
              ...
              <div className="bg-slate-50 p-5 rounded-xl border border-slate-200 font-sans text-[10pt] space-y-2 print:bg-transparent print:border-black break-inside-avoid">
                 <p>Telah melakukan pengambilan {data.donorType.toLowerCase()} pada:</p>
                 <div className="grid grid-cols-[140px_10px_1fr]"><span>Hari, Tanggal</span><span>:</span><span className="font-bold">{new Date(data.date + 'T00:00:00').toLocaleDateString('id-ID', {weekday: 'long', day: 'numeric', month: 'long', year: 'numeric'})}</span></div>
                 <div className="grid grid-cols-[140px_10px_1fr]"><span>Waktu / Lokasi</span><span>:</span><span>{data.donorTime} / {data.location}</span></div>
              </div>

              <p className="text-justify indent-8 leading-relaxed break-inside-avoid">
                 {data.statement}
              </p>

              <p className="break-inside-avoid">Demikian surat keterangan ini dibuat agar dapat dipergunakan sebagaimana mestinya, termasuk sebagai bukti ijin istirahat bagi instansi/tempat bekerja.</p>
            </div>

            {/* TANDA TANGAN */}
            <div className="mt-12 flex justify-between items-end shrink-0 break-inside-avoid" style={{ pageBreakInside: 'avoid' }}>
        ...
        {templateId === 2 && (
          <div className="font-sans text-[10.5pt] text-slate-800 leading-snug flex flex-col h-full">
            ...
            <div className="mb-8 flex-grow">
              <h2 className="text-xl font-bold uppercase text-slate-900 mb-6 break-inside-avoid">Laporan Donasi Medis</h2>
              ...
              <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 mb-6 print:bg-transparent print:border-black break-inside-avoid">
              ...
              <p className="text-justify text-sm leading-relaxed break-inside-avoid">
            ...
            <div className="mt-12 pt-4 border-t border-slate-900 flex justify-between items-end shrink-0 break-inside-avoid" style={{ pageBreakInside: 'avoid' }}>
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans text-slate-900 print:bg-white print:m-0">
      ...
      <div className="no-print bg-slate-900 text-white shadow-lg sticky top-0 z-50 border-b border-slate-700 h-16 font-sans shrink-0">
        <div className="max-w-[1600px] mx-auto px-4 h-full flex justify-between items-center text-sm">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center gap-2 text-sm font-bold text-slate-300 uppercase tracking-tight">
               <Heart size={16} className="text-red-500" /> <span>Donor Keterangan</span>
            </div>
          </div>
          <div className="flex items-center gap-4">
      ...
      <main className="flex-grow flex flex-col md:flex-row overflow-hidden h-[calc(100vh-64px)] relative print:hidden print:h-auto print:overflow-visible">
        ...
        <div className={`no-print w-full lg:w-[450px] bg-slate-50 border-r border-slate-200 flex flex-col h-full z-10 transition-transform duration-300 absolute lg:relative shadow-xl lg:shadow-none ${mobileView === 'preview' ? '-translate-x-full lg:translate-x-0' : 'translate-x-0'}`}>
           <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-white sticky top-0 z-10">
                <h2 className="font-bold text-slate-700 flex items-center gap-2"><Edit3 size={16} /> Edit Data</h2>
           ...
           <div className="flex-1 overflow-y-auto p-6 space-y-8 pb-32 md:pb-10 custom-scrollbar print:hidden print:overflow-visible print:bg-white">
              <div className="space-y-3">
                 ...
                 <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3">
                    <input className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm font-bold uppercase focus:ring-2 focus:ring-emerald-500 outline-none" value={data.institutionName} onChange={e => handleDataChange('institutionName', e.target.value)} placeholder="Nama Instansi" />
                    <textarea className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs h-16 resize-none focus:ring-2 focus:ring-emerald-500 outline-none" value={data.institutionAddress} onChange={e => handleDataChange('institutionAddress', e.target.value)} placeholder="Alamat Instansi" />
                 </div>
              </div>
              ...
              <div className="space-y-3">
                 ...
                 <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3">
                    <input className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm font-bold uppercase focus:ring-2 focus:ring-emerald-500 outline-none" value={data.donorName} onChange={e => handleDataChange('donorName', e.target.value)} placeholder="Nama Lengkap" />
                    <div className="grid grid-cols-2 gap-3">
                       <input className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs focus:ring-2 focus:ring-emerald-500 outline-none" value={data.donorNik} onChange={e => handleDataChange('donorNik', e.target.value)} placeholder="NIK" />
                       <input className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs font-black text-red-600 focus:ring-2 focus:ring-emerald-500 outline-none" value={data.bloodType} onChange={e => handleDataChange('bloodType', e.target.value)} placeholder="Gol. Darah" />
                    </div>
                 </div>
              </div>
              ...
              <div className="space-y-3">
                 ...
                 <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3">
                    <input className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs focus:ring-2 focus:ring-emerald-500 outline-none" value={data.donorType} onChange={e => handleDataChange('donorType', e.target.value)} placeholder="Jenis Donor" />
                    <div className="grid grid-cols-2 gap-3">
                       <input className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs focus:ring-2 focus:ring-emerald-500 outline-none" value={data.donorTime} onChange={e => handleDataChange('donorTime', e.target.value)} placeholder="Waktu" />
                    ...
                    <input className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs focus:ring-2 focus:ring-emerald-500 outline-none" value={data.location} onChange={e => handleDataChange('location', e.target.value)} placeholder="Lokasi" />
                 </div>
              </div>
              ...
              <div className="space-y-3">
                 ...
                 <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3">
                    <textarea className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs h-20 resize-none focus:ring-2 focus:ring-emerald-500 outline-none" value={data.statement} onChange={e => handleDataChange('statement', e.target.value)} placeholder="Catatan Medis" />
                    <div className="grid grid-cols-2 gap-3">
                       <input className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs font-bold focus:ring-2 focus:ring-emerald-500 outline-none" value={data.officerName} onChange={e => handleDataChange('officerName', e.target.value)} placeholder="Nama Petugas" />
                       <input className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs focus:ring-2 focus:ring-emerald-500 outline-none" value={data.officerId} onChange={e => handleDataChange('officerId', e.target.value)} placeholder="NIP/ID" />
                    </div>
                    <input className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs focus:ring-2 focus:ring-emerald-500 outline-none" value={data.city} onChange={e => handleDataChange('city', e.target.value)} placeholder="Kota Surat" />
                 </div>
              </div>
        ...
        {/* PREVIEW AREA */}
        <div className={`no-print flex-1 bg-slate-200/50 relative overflow-hidden flex flex-col items-center ${mobileView === 'editor' ? 'hidden lg:flex' : 'flex'} print:hidden print:overflow-visible print:bg-white print:static`}>
            <div className="flex-1 overflow-y-auto w-full flex justify-center p-4 md:p-8 custom-scrollbar print:hidden print:overflow-visible print:bg-white">
               <div className="origin-top transition-transform duration-300 transform scale-[0.55] md:scale-[0.85] lg:scale-100 mb-[-130mm] md:mb-[-20mm] lg:mb-0 shadow-2xl flex flex-col items-center print:scale-100 print:transform-none print:w-full print:m-0 print:block">
      ...
      {/* MOBILE NAV */}
      <div className="no-print lg:hidden fixed bottom-6 left-6 right-6 z-50 h-14 bg-slate-900/90 backdrop-blur-md rounded-2xl shadow-2xl border border-white/10 flex p-1.5 font-sans">
