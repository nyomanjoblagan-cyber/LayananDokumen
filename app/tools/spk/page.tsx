...
import { 
  ...
  LayoutTemplate, ChevronDown, Check, ImagePlus, X, RotateCcw, ArrowLeftCircle
} from 'lucide-react';
...
export default function PerintahBayarPage() {
  return (
    <Suspense fallback={<div className="flex h-screen items-center justify-center text-slate-400 text-xs bg-slate-50 uppercase tracking-widest font-bold">Memuat Editor SPB...</div>}>
      <PaymentOrderBuilder />
    </Suspense>
  );
}
...
function PaymentOrderBuilder() {
  ...
  const handleReset = () => {
    if(typeof window !== 'undefined' && window.confirm('Reset semua data pembayaran?')) {
        const today = new Date().toISOString().split('T')[0];
  ...
  const DocumentContent = () => (
    <div className={`bg-white flex flex-col box-border text-slate-900 leading-normal p-[15mm] md:p-[20mm] print:p-0 w-[210mm] min-h-[296mm] shadow-2xl print:shadow-none print:m-0 mx-auto ${templateId === 1 ? 'font-serif text-[11pt]' : 'font-sans text-[10.5pt]'}`}>
      ...
      {/* HEADER KOP */}
      <div className="flex items-center gap-6 border-b-4 border-double border-slate-900 pb-4 mb-8 shrink-0 text-left font-sans">
        <div className="flex-shrink-0">
          {logo ? (
            <img src={logo} alt="Logo" className="w-20 h-20 object-contain block" />
          ) : (
      ...
      {/* JUDUL */}
      <div className="text-center mb-10 shrink-0 leading-tight font-sans">
      ...
      <div className="space-y-8 flex-grow overflow-visible text-left leading-relaxed">
        <div className="font-medium">
            <p>Kepada Yth. <b>{data.treasurerName}</b></p>
            <p className="text-slate-500 print:text-black uppercase text-xs font-bold tracking-widest">{data.treasurerJob}</p>
        </div>

        <p>Dengan hormat, harap segera melakukan realisasi pembayaran dana tunai/transfer dengan rincian instruksi sebagai berikut:</p>
        ...
        <div className="bg-slate-50 p-6 rounded-2xl border-2 border-slate-100 space-y-4 print:bg-transparent print:border-2 print:border-black break-inside-avoid">
           ...
           <div className="pt-3 border-t border-dashed border-slate-200 print:border-black text-[9.5pt]">
              ...
              <div className="grid grid-cols-[160px_10px_1fr]"><span>Nomor Rekening</span><span>:</span><span className="font-mono font-black text-blue-700 print:text-black tracking-widest">{data.recipientAccount}</span></div>
           </div>
        </div>
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans text-slate-900">
      ...
      <div className="no-print bg-slate-900 text-white shadow-lg sticky top-0 z-50 h-16 flex items-center justify-between px-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => { window.print(); setShowDonation(true); }} className="bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-1.5 rounded-lg font-bold text-xs uppercase shadow-lg active:scale-95 transition-all flex items-center gap-2">
      ...
      <main className="flex-grow flex flex-col md:flex-row overflow-hidden h-[calc(100vh-64px)]">
        ...
        <div className={`no-print w-full md:w-[450px] bg-white border-r flex flex-col h-full absolute md:relative z-10 transition-transform ${mobileView === 'preview' ? '-translate-x-full md:translate-x-0' : 'translate-x-0'}`}>
           ...
           <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar pb-32">
              <div className="bg-white rounded-xl shadow-sm border p-4 space-y-4 font-sans text-left">
              ...
              <div className="bg-white rounded-xl shadow-sm border p-4 space-y-4 font-sans text-left">
                <h3 className="text-[10px] font-black uppercase text-emerald-600 border-b pb-1 tracking-widest flex items-center gap-2">
                  <Wallet size={12}/> Detail Pembayaran
                </h3>
                <input className="w-full p-2 border rounded-lg text-xs font-bold uppercase focus:ring-2 focus:ring-emerald-500 outline-none" value={data.recipientName} onChange={e => handleDataChange('recipientName', e.target.value)} placeholder="Nama Penerima" />
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <input className="w-full p-2 border rounded-lg text-[10px] focus:ring-2 focus:ring-emerald-500 outline-none uppercase" value={data.recipientBank} onChange={e => handleDataChange('recipientBank', e.target.value)} placeholder="Nama Bank" />
                </div>
                ...
                <textarea className="w-full p-2 border rounded-lg text-xs h-20 resize-none focus:ring-2 focus:ring-emerald-500 outline-none leading-relaxed" value={data.amountText} onChange={e => handleDataChange('amountText', e.target.value)} placeholder="Terbilang..." />
              </div>

              <div className="bg-white rounded-xl shadow-sm border p-4 space-y-4 font-sans text-left">
                <h3 className="text-[10px] font-black uppercase text-amber-600 border-b pb-1 tracking-widest flex items-center gap-2">
                  <Receipt size={12}/> Otoritas & Administrasi
                </h3>
                <textarea className="w-full p-2 border rounded-lg text-xs h-24 focus:ring-2 focus:ring-amber-500 outline-none leading-relaxed" value={data.purpose} onChange={e => handleDataChange('purpose', e.target.value)} placeholder="Keperluan Pembayaran..." />
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <input className="w-full p-2 border rounded-lg text-xs font-bold uppercase" value={data.approverName} onChange={e => handleDataChange('approverName', e.target.value)} placeholder="Penyetuju" />
                  <input className="w-full p-2 border rounded-lg text-xs font-bold uppercase" value={data.treasurerName} onChange={e => handleDataChange('treasurerName', e.target.value)} placeholder="Bendahara" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <input className="w-full p-2 border rounded-lg text-[10px] font-mono" value={data.docNo} onChange={e => handleDataChange('docNo', e.target.value)} placeholder="No. SPB" />
                </div>
              </div>
           </div>
        </div>
