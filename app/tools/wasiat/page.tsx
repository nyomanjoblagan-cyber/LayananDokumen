 ...
 * STATUS: FINAL & FIXED PREVIEW (2 Pages)
 * DESC: Generator Surat Wasiat Profesional dengan Daftar Ahli Waris Dinamis
 */
...
import { 
  ...
  ChevronDown, Check, LayoutTemplate, Edit3, Eye, Gavel, Scroll, RotateCcw
} from 'lucide-react';
...
// --- 2. TEMPLATE DEFINITIONS (Global access to prevent build errors) ---
const TEMPLATES = [
...
function WillLetterBuilder() {
  ...
  const handleReset = () => {
    if(confirm('Reset semua data wasiat?')) {
  ...
  const DocumentContent = () => (
    <div className={`bg-white text-slate-900 leading-relaxed ${templateId === 1 ? 'font-serif text-[11pt]' : 'font-sans text-[10.5pt]'} print:text-black`}>
      
      {/* HALAMAN 1 */}
      <div className="w-[210mm] h-[296mm] p-[20mm] md:p-[25mm] bg-white shadow-2xl print:shadow-none box-border flex flex-col mb-10 print:mb-0 print:break-after-page overflow-hidden border-b print:border-none relative text-left">
        ...
        <div className="text-justify space-y-6 overflow-visible">
          <p>Saya yang bertanda tangan di bawah ini (selanjutnya disebut sebagai <b>TESTATOR</b>):</p>
          <div className={`ml-8 space-y-1 font-sans border-l-4 ${templateId === 1 ? 'border-slate-100' : 'border-blue-600'} pl-6 italic print:border-slate-300 text-[10pt]`}>
              ...
              <div className="grid grid-cols-[160px_10px_1fr] align-top"><span>Alamat</span><span>:</span><span>{data.testatorAddress}</span></div>
          </div>

          <div className="space-y-3">
            <h3 className={`font-bold uppercase text-[9pt] border-b pb-1 ${templateId === 2 ? 'text-blue-700 border-blue-100' : 'border-slate-900'}`}>PASAL 1: PERNYATAAN STATUS</h3>
          ...
          <div className="space-y-3">
            <h3 className={`font-bold uppercase text-[9pt] border-b pb-1 ${templateId === 2 ? 'text-blue-700 border-blue-100' : 'border-slate-900'}`}>PASAL 2: PEMBATALAN WASIAT</h3>
          ...
          <div className="space-y-3">
            <h3 className={`font-bold uppercase text-[9pt] border-b pb-1 ${templateId === 2 ? 'text-blue-700 border-blue-100' : 'border-slate-900'}`}>PASAL 3: PENETAPAN AHLI WARIS</h3>
        ...
        <p className="mt-auto text-center italic text-slate-400 text-[8pt] pb-4">--- Bersambung ke halaman berikutnya ---</p>
      </div>

      {/* HALAMAN 2 */}
      <div className="w-[210mm] h-[296mm] p-[20mm] md:p-[25mm] bg-white shadow-2xl print:shadow-none box-border flex flex-col relative overflow-hidden text-left">
        <div className="text-justify space-y-6 overflow-visible">
          <div className="space-y-3">
            <h3 className={`font-bold uppercase text-[9pt] border-b pb-1 ${templateId === 2 ? 'text-blue-700 border-blue-100' : 'border-slate-900'}`}>PASAL 4: PELAKSANA WASIAT</h3>
          ...
          <div className="space-y-3">
            <h3 className={`font-bold uppercase text-[9pt] border-b pb-1 ${templateId === 2 ? 'text-blue-700 border-blue-100' : 'border-slate-900'}`}>PASAL 5: AMANAT KHUSUS</h3>
            <div className={`${templateId === 1 ? 'bg-slate-50' : 'bg-blue-50'} p-5 rounded-xl border ${templateId === 1 ? 'border-slate-200' : 'border-blue-100'} print:bg-transparent print:border-black italic text-[10pt] leading-relaxed`}>
              "{data.specialMessage}"
            </div>
          </div>

          <div className="space-y-3">
            <h3 className={`font-bold uppercase text-[9pt] border-b pb-1 ${templateId === 2 ? 'text-blue-700 border-blue-100' : 'border-slate-900'}`}>PASAL 6: PENUTUP</h3>
        ...
        {/* TANDA TANGAN */}
        <div className={`mt-auto pt-8 border-t-2 ${templateId === 1 ? 'border-slate-900' : 'border-blue-600 print:border-black'}`}>
           <div className="grid grid-cols-2 gap-x-10 text-center mb-10">
              <div className="flex flex-col items-center">
                <p className="font-black uppercase text-[8pt] mb-16 tracking-widest text-slate-400 print:text-black">Saksi I,</p>
                <p className="font-bold underline uppercase text-[10pt]">({data.witness1})</p>
              </div>
              <div className="flex flex-col items-center">
                <p className="font-black uppercase text-[8pt] mb-16 tracking-widest text-slate-400 print:text-black">Saksi II,</p>
           ...
           <div className="flex flex-col items-center">
              <div className="text-center w-72">
                 <p className="font-black uppercase text-[8pt] mb-4 tracking-[0.4em] text-blue-600 print:text-black">Pembuat Wasiat,</p>
                 <div className="border border-slate-200 w-24 h-14 mx-auto mb-4 flex items-center justify-center text-[7pt] text-slate-300 italic uppercase print:border-black print:text-black">Materai 10.000</div>
                 <p className={`font-bold underline uppercase text-[12pt] leading-none ${templateId === 1 ? 'italic' : ''}`}>{data.testatorName}</p>
              </div>
           </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-100 font-sans text-slate-900 print:bg-white print:m-0">
        ...
        <div className="max-w-[1600px] mx-auto px-4 h-full flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <h1 className="hidden md:flex items-center gap-2 text-sm font-bold text-amber-400 uppercase tracking-tighter italic">
               <Gavel size={16} /> <span>Will Builder Pro</span>
            </h1>
          </div>
          ...
          <div className="flex items-center gap-3 text-left">
            ...
            <button onClick={() => window.print()} className="bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2 rounded-xl font-bold text-xs uppercase shadow-lg active:scale-95 transition-all flex items-center gap-2 font-sans">
      ...
      <main className="flex flex-col md:flex-row h-[calc(100vh-64px)] overflow-hidden relative">
        {/* SIDEBAR */}
        <div className={`no-print w-full lg:w-[480px] bg-white border-r overflow-y-auto p-4 md:p-6 space-y-6 z-20 h-full ${mobileView === 'preview' ? 'hidden lg:block' : 'block'}`}>
           <div className="flex justify-between items-center border-b pb-2 text-left font-sans">
                <h2 className="font-bold text-slate-700 uppercase text-xs tracking-widest flex items-center gap-2"><Edit3 size={16}/> Editor Wasiat</h2>
           ...
           <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 space-y-4 font-sans text-left">
              ...
              <input className="w-full p-2 border rounded text-xs font-bold uppercase bg-slate-50" value={data.testatorName} onChange={e => handleDataChange('testatorName', e.target.value)} placeholder="Nama Pembuat Wasiat" />
              <input className="w-full p-2 border rounded text-xs" value={data.testatorNik} onChange={e => handleDataChange('testatorNik', e.target.value)} placeholder="NIK / No. KTP" />
              <textarea className="w-full p-2 border rounded text-xs h-16 resize-none" value={data.testatorAddress} onChange={e => handleDataChange('testatorAddress', e.target.value)} placeholder="Alamat Domisili" />
              <input className="w-full p-2 border border-indigo-200 rounded text-xs font-bold text-indigo-600" value={data.executorName} onChange={e => handleDataChange('executorName', e.target.value)} placeholder="Nama Pelaksana Wasiat" />
           </div>
           ...
           <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 space-y-4 font-sans text-left">
              <div className="flex justify-between items-center border-b pb-1">
                 ...
                 <button onClick={addBeneficiary} className="bg-emerald-100 text-emerald-700 p-1 px-2 rounded-md text-[9px] font-bold">+ TAMBAH</button>
              </div>
              <div className="space-y-3">
                 {data.beneficiaries.map((b, idx) => (
                    <div key={idx} className="p-3 bg-slate-50 rounded-xl border relative group space-y-2">
                       <button onClick={() => removeBeneficiary(idx)} className="absolute top-2 right-2 text-slate-300 hover:text-red-500 transition-colors"><Trash2 size={14}/></button>
                       <input className="w-full p-1 border-b bg-transparent text-xs font-bold focus:outline-none" value={b.name} onChange={e => updateBeneficiary(idx, 'name', e.target.value)} placeholder="Nama Ahli Waris" />
                       <textarea className="w-full p-1 text-[10px] h-14 bg-white border rounded resize-none" value={b.item} onChange={e => updateBeneficiary(idx, 'item', e.target.value)} placeholder="Item Harta (Cth: Rumah, Tanah, Emas)" />
                    </div>
                 ))}
              </div>
           </div>
           ...
           <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 space-y-4 font-sans text-left pb-10">
              <h3 className="text-[10px] font-black uppercase text-rose-600 border-b pb-1 flex items-center gap-2"><Heart size={12}/> Amanat & Saksi</h3>
              <textarea className="w-full p-2 border rounded text-xs h-20 resize-none italic bg-slate-50" value={data.specialMessage} onChange={e => handleDataChange('specialMessage', e.target.value)} placeholder="Pesan Terakhir / Amanat" />
              <div className="grid grid-cols-2 gap-2">
                 <input className="p-2 border rounded text-xs" value={data.witness1} onChange={e => handleDataChange('witness1', e.target.value)} placeholder="Saksi I" />
                 <input className="p-2 border rounded text-xs" value={data.witness2} onChange={e => handleDataChange('witness2', e.target.value)} placeholder="Saksi II" />
              </div>
              <div className="grid grid-cols-2 gap-2">
                 <input className="p-2 border rounded text-xs" value={data.city} onChange={e => handleDataChange('city', e.target.value)} placeholder="Kota" />
                 <input type="date" className="p-2 border rounded text-xs" value={data.date} onChange={e => handleDataChange('date', e.target.value)} />
              </div>
           </div>
        ...
        <div className={`flex-1 bg-slate-200/50 relative h-full no-print flex flex-col items-center ${mobileView === 'editor' ? 'hidden lg:flex' : 'flex'}`}>
            <div className="flex-1 overflow-y-auto w-full flex flex-col items-center p-4 md:p-8 custom-scrollbar">
               <div className="origin-top transition-transform duration-300 transform scale-[0.45] sm:scale-[0.55] md:scale-[0.8] lg:scale-100 h-fit mb-12 shadow-2xl flex flex-col items-center">
                 <DocumentContent />
               </div>
            </div>
        </div>
