    ...
    if (templateId === 1) {
      ...
      return (
        <div className="font-serif text-[11pt] leading-relaxed text-justify text-black">
          ...
          <p className="mb-4">
            Pada hari ini, <b>{formatDateSafe(data.date)}</b>, bertempat di {data.city}, kami yang bertanda tangan di bawah ini:
          </p>
          
          <div className="mb-6 pl-4 space-y-4">
              <div className="flex break-inside-avoid">
                  <span className="w-8 font-bold text-slate-500">1.</span>
      ...
      return (
        <div className="font-sans text-[10pt] leading-snug text-slate-800">
          ...
          <div className="bg-slate-50 px-4 py-3 rounded-lg border border-slate-200 mb-6 text-xs flex justify-between break-inside-avoid">
              <p>Tanggal: <b>{formatDateSafe(data.date)}</b></p>
          ...
          <div className="flex items-end justify-between pt-8 border-t-2 border-slate-900 mt-auto break-inside-avoid" style={{ pageBreakInside: 'avoid' }}>
              <div className="text-center w-1/3">
                 <p className="text-[9px] font-bold text-slate-400 uppercase mb-16">Franchisor</p>
                 <p className="font-bold border-b border-slate-900 pb-1 text-xs uppercase">{data.p1Name}</p>
                 <p className="text-[8px] text-slate-500 mt-1">{data.p1Title}</p>
              </div>
              <div className="text-center w-1/4">
                 <p className="text-[9px] font-bold text-slate-400 uppercase mb-4 tracking-widest">Saksi</p>
                 <div className="space-y-8 text-[9px]">
                    <p className="border-b border-slate-300 pb-1 uppercase">{data.witness1}</p>
                    <p className="border-b border-slate-300 pb-1 uppercase">{data.witness2}</p>
                 </div>
              </div>
              <div className="text-center w-1/3">
                 <p className="text-[9px] font-bold text-slate-400 uppercase mb-16">Franchisee</p>
                 <p className="font-bold border-b border-slate-900 pb-1 text-xs uppercase">{data.p2Name}</p>
                 <p className="text-[8px] text-slate-500 mt-1">Pemilik Waralaba</p>
              </div>
          </div>
        </div>
      );
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8 pb-32 md:pb-10 custom-scrollbar">
               <div className="space-y-3">
                  ...
                  <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                      <input className="w-full p-2 border rounded text-xs font-bold font-mono focus:ring-2 focus:ring-emerald-500 outline-none" value={data.docNo} onChange={e => handleDataChange('docNo', e.target.value)} placeholder="Nomor Perjanjian" />
                      <div className="grid grid-cols-2 gap-3">
                         <input type="text" className="w-full p-2 border rounded text-xs focus:ring-2 focus:ring-emerald-500 outline-none" value={data.city} onChange={e => handleDataChange('city', e.target.value)} placeholder="Kota" />
                         <input type="date" className="w-full p-2 border rounded text-xs focus:ring-2 focus:ring-emerald-500 outline-none" value={data.date} onChange={e => handleDataChange('date', e.target.value)} />
                      </div>
                  </div>
               </div>
