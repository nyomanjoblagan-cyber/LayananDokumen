...
import { 
  ...
  ChevronDown, Check, Edit3, Eye, ImagePlus, RotateCcw, Scale, Scroll, Gavel, FileText
} from 'lucide-react';
...
function LandSafetyBuilder() {
  ...
  const handleReset = () => {
    if(confirm('Reset semua data tanah?')) {
  ...
  const DocumentContent = () => (
    <div className={`text-slate-900 leading-relaxed print:text-black ${templateId === 2 ? 'font-sans' : 'font-serif'}`}>
      
      {/* HALAMAN 1 */}
      <div className="w-[210mm] h-[296mm] p-[20mm] md:p-[25mm] bg-white shadow-2xl print:shadow-none box-border flex flex-col mb-10 print:mb-0 print:break-after-page overflow-hidden relative border-b print:border-none">
        {/* KOP */}
        <div className={`flex items-center gap-6 ${templateId === 1 ? 'border-b-4 border-double border-black' : 'border-b border-slate-200'} pb-4 mb-8 text-center shrink-0`}>
          {logo && <img src={logo} alt="Logo" className="w-16 h-16 object-contain block print:block" />}
          <div className="flex-grow text-center">
            <div className={`text-[12pt] font-black leading-tight uppercase ${templateId === 1 ? 'italic' : ''} whitespace-pre-line print:text-black`}>
              {data.issuerOffice}
            </div>
          </div>
        </div>
        ...
        <div className="text-center mb-8 shrink-0">
          <h2 className="text-xl font-black underline uppercase decoration-1 underline-offset-4 tracking-widest print:text-black">SURAT PERNYATAAN FISIK & TIDAK SENGKETA</h2>
          <p className="text-[10pt] font-sans mt-1 font-bold print:text-black">NOMOR: {data.docNo}</p>
        </div>

        {/* BODY HALAMAN 1 */}
        <div className="text-justify text-[10pt] space-y-5 overflow-visible">
          ...
          <div className="space-y-2 pt-2">
            <h3 className="font-bold uppercase text-[9pt] border-b pb-1 print:border-black">PASAL 1: BATAS-BATAS BIDANG TANAH</h3>
            <div className="grid grid-cols-2 gap-x-8 gap-y-1 ml-4 font-sans text-[9pt]">
          ...
          <div className="space-y-2">
            <h3 className="font-bold uppercase text-[9pt] border-b pb-1 print:border-black">PASAL 2: PERNYATAAN STATUS HUKUM</h3>
            <ol className="list-decimal ml-10 space-y-1">
              ...
              <li>Benar tanah tersebut dikuasai secara fisik oleh Penyata secara terus-menerus.</li>
            </ol>
          </div>
          <p className="text-center italic text-slate-400 text-[8pt] pt-10">--- Bersambung ke halaman berikutnya ---</p>
        </div>
      </div>

      {/* HALAMAN 2 */}
      <div className="w-[210mm] h-[296mm] p-[20mm] md:p-[25mm] bg-white shadow-2xl print:shadow-none box-border flex flex-col relative overflow-hidden">
        <div className="text-justify text-[10pt] space-y-6 overflow-visible">
          <div className="space-y-4">
            <ol className="list-decimal ml-10 space-y-2 font-sans" start={3}>
          ...
          <div className="space-y-3 pt-4">
            <h3 className="font-bold uppercase text-[9pt] border-b pb-1 print:border-black">PASAL 3: KLAUSUL PENJAMINAN & GANTI RUGI</h3>
          ...
          <div className="space-y-3 pt-4">
            <h3 className="font-bold uppercase text-[9pt] border-b pb-1 print:border-black">PASAL 4: PENUTUP</h3>
        ...
        {/* AREA TANDA TANGAN DI HALAMAN 2 */}
        <div className="mt-auto border-t-2 border-black pt-6">
           ...
           <div className="flex flex-col items-center">
              <div className="text-center w-64 mb-6">
                 ...
                 <div className="border border-slate-200 w-24 h-12 mx-auto mb-2 flex items-center justify-center text-[7pt] italic uppercase print:border-black">Materai 10.000</div>
      ...
      <div className="no-print bg-slate-900 text-white shadow-lg sticky top-0 z-50 h-16 border-b border-slate-700">
        <div className="max-w-[1600px] mx-auto px-4 h-full flex justify-between items-center text-sm">
          <div className="flex items-center gap-4">
            ...
            <h1 className="hidden md:block font-bold text-blue-400 uppercase tracking-tighter italic">Legal Land Safety Builder</h1>
          </div>
      ...
      <main className="flex flex-col md:flex-row h-[calc(100vh-64px)] overflow-hidden relative">
        ...
        <div className={`no-print w-full lg:w-[480px] bg-white border-r overflow-y-auto p-4 md:p-6 space-y-6 z-20 h-full ${mobileView === 'preview' ? 'hidden lg:block' : 'block'}`}>
           <div className="flex justify-between items-center border-b pb-2 text-left">
                <h2 className="font-bold text-slate-700 uppercase text-xs tracking-widest flex items-center gap-2 font-sans"><Edit3 size={16}/> Editor Pernyataan</h2>
           ...
           <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 space-y-4 font-sans text-left">
              ...
              <div className="flex items-center gap-4">
                 ...
                 <textarea className="flex-1 p-2 border rounded text-xs font-bold uppercase bg-slate-50 h-16 leading-tight" value={data.issuerOffice} onChange={e => handleDataChange('issuerOffice', e.target.value)} />
              </div>
           ...
           <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 space-y-4 font-sans text-left">
              <h3 className="text-[10px] font-black uppercase text-emerald-600 border-b pb-1 flex items-center gap-2"><UserCircle2 size={12}/> Data Pemilik</h3>
              ...
              <textarea className="w-full p-2 border rounded text-xs h-16" value={data.ownerAddress} onChange={e => handleDataChange('ownerAddress', e.target.value)} placeholder="Alamat Sesuai KTP" />
           </div>

           <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 space-y-4 font-sans text-left">
              <h3 className="text-[10px] font-black uppercase text-amber-600 border-b pb-1 flex items-center gap-2"><MapPin size={12}/> Detail Tanah</h3>
              <input className="w-full p-2 border rounded text-xs" value={data.landLocation} onChange={e => handleDataChange('landLocation', e.target.value)} placeholder="Lokasi Objek" />
              <div className="grid grid-cols-2 gap-2">
                 <input className="p-2 border rounded text-xs font-bold" value={data.landSize} onChange={e => handleDataChange('landSize', e.target.value)} placeholder="Luas ±" />
                 <input className="p-2 border rounded text-xs" value={data.landStatus} onChange={e => handleDataChange('landStatus', e.target.value)} placeholder="Alas Hak" />
              </div>
           </div>
           ...
           <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 space-y-4 font-sans text-left">
              ...
              <div className="grid grid-cols-2 gap-2">
                 <input className="p-2 border rounded text-[10px]" value={data.borderNorth} onChange={e => handleDataChange('borderNorth', e.target.value)} placeholder="Utara" />
                 <input className="p-2 border rounded text-[10px]" value={data.borderSouth} onChange={e => handleDataChange('borderSouth', e.target.value)} placeholder="Selatan" />
                 <input className="p-2 border rounded text-[10px]" value={data.borderEast} onChange={e => handleDataChange('borderEast', e.target.value)} placeholder="Timur" />
                 <input className="p-2 border rounded text-[10px]" value={data.borderWest} onChange={e => handleDataChange('borderWest', e.target.value)} placeholder="Barat" />
              </div>
           </div>

           <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 space-y-4 font-sans text-left pb-16">
              ...
              <div className="grid grid-cols-2 gap-2">
                <input className="p-2 border rounded text-xs" value={data.witness1} onChange={e => handleDataChange('witness1', e.target.value)} placeholder="Saksi I (RT/RW)" />
                <input className="p-2 border rounded text-xs" value={data.witness2} onChange={e => handleDataChange('witness2', e.target.value)} placeholder="Saksi II (Tokoh)" />
              </div>
              <input className="w-full p-2 border rounded text-xs font-bold" value={data.villageHead} onChange={e => handleDataChange('villageHead', e.target.value)} placeholder="Nama Kades/Lurah" />
        ...
        <div className={`flex-1 bg-slate-200/50 relative h-full no-print flex flex-col items-center ${mobileView === 'editor' ? 'hidden lg:flex' : 'flex'}`}>
            <div className="flex-1 overflow-y-auto w-full flex flex-col items-center p-4 md:p-8 custom-scrollbar">
               <div className="origin-top transition-transform duration-300 transform scale-[0.43] sm:scale-[0.55] md:scale-[0.75] lg:scale-100 h-fit mb-12 shadow-2xl flex flex-col items-center">
                 <DocumentContent />
               </div>
            </div>
        </div>
