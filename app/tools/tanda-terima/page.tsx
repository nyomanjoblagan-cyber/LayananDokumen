...
import { 
  ...
  ChevronDown, Check, LayoutTemplate, Edit3, Eye, ImagePlus
} from 'lucide-react';
...
function ReceiptBuilder() {
  ...
  const DocumentContent = () => (
    <div className={`bg-white flex flex-col box-border text-slate-900 leading-normal p-[15mm] md:p-[20mm] w-[210mm] min-h-[296mm] shadow-2xl print:shadow-none print:m-0 print:h-auto print:min-h-0 print:p-[15mm] ${templateId === 1 ? 'font-sans' : 'font-serif text-[11pt]'}`}>
      ...
      <div className="flex-grow overflow-hidden">
        <table className="w-full border-collapse border border-slate-900 text-[10pt]">
           ...
           <tbody>
             ...
             {/* Filler Rows to keep 1 page look */}
      ...
      {/* TANDA TANGAN */}
      <div className="shrink-0 mt-8 pt-4 border-t-2 border-slate-100 print:border-slate-900">
      ...
      <main className="flex-grow flex flex-col md:flex-row overflow-hidden h-[calc(100vh-64px)] relative font-sans">
        ...
        <div className={`no-print w-full lg:w-[450px] bg-white border-r overflow-y-auto p-4 md:p-6 space-y-6 z-20 h-full ${mobileView === 'preview' ? 'hidden lg:block' : 'block'}`}>
           <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 space-y-4 font-sans text-left">
              ...
              <div className="flex items-center gap-4">
                 {logo ? (
                    <div className="relative w-14 h-14 border rounded overflow-hidden group">
              ...
              <div className="space-y-3">
                 {data.items.map((item, idx) => (
                    <div key={idx} className="p-3 bg-slate-50 rounded-xl border relative group space-y-2">
                       <button onClick={() => removeItem(idx)} className="absolute top-2 right-2 text-red-400 hover:text-red-600 transition-colors"><Trash2 size={14}/></button>
                       ...
                       <div className="grid grid-cols-3 gap-2">
                          <input className="w-full p-1 border rounded text-[10px] bg-white" value={item.qty} onChange={e => updateItem(idx, 'qty', e.target.value)} placeholder="Qty" />
                          <input className="w-full p-1 border rounded text-[10px] bg-white" value={item.unit} onChange={e => updateItem(idx, 'unit', e.target.value)} placeholder="Unit" />
           ...
           <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 space-y-4 font-sans text-left pb-10">
              ...
              <div className="grid grid-cols-2 gap-2">
                 ...
                 <input className="w-full p-2 border rounded text-[10px]" value={data.receiverSignName} onChange={e => handleDataChange('receiverSignName', e.target.value)} placeholder="Nama Penerima (TTD)" />
              </div>
           </div>
